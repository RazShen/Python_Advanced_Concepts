...
def main():
    ...
    with open("testfile.txt") as fp:
       for line in iter(fp.readline, ''):  # when the value called by fp.readline is equal to ''- the iterator will stop
            print(line)
    with open("testfile.txt") as fp:
        print(list(iter(fp.readline,''))) #when the value called by fp.readline is equal to ''- the iterator will stop
